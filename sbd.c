...
#define SIMP_BLKDEV_SECTORSIZE        (1UL<<SIMP_BLKDEV_SECTORSHIFT)
...
static unsigned int simp_blkdev_make_request(struct request_queue *q, struct bio *bio)
{
	...
	default:
		printk(KERN_ERR SIMP_BLKDEV_DISKNAME
			": unknown valuunsigned e of bio_rw: %lu\n", bio_rw(bio));
		goto bio_err;
	}

	if ((bio->bi_iter.bi_sector << SIMP_BLKDEV_SECTORSHIFT) + bio->bi_iter.bi_size
			> simp_blkdev_bytes) {
		printk(KERN_ERR SIMP_BLKDEV_DISKNAME
